...
class ConsumeCommandTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testItConsumes()
    {
        ...
        $consumer->expects($this->once())->method('consume')->with($this->equalTo($queue), $this->equalTo(null), $this->equalTo(array(
            'max_retries' => 5,
            'max_runtime' => null,
        )));
        ...
        $input->expects($this->once())->method('getArgument')->with($this->equalTo('queue'))->will($this->returnValue('send-newsletter'));
